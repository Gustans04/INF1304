 ...
 * This class implements a WebSocket server that handles incoming messages and broadcasts
...
public class WebSocketServer {
    ...
    /// Instância do ChatProducer para enviar mensagens para o Kafka.
 private Session session;
    /// Instância da sessão WebSocket associada a esta conexão.
 ...
 */
 public static void startServer(ChatProducer producer) {
    ...
    /**
     * Stops the WebSocket server and releases any associated resources. This
     * method should be called when the application is shutting down. It ensures
     * that the WebSocket server is properly stopped and any active connections
     * are closed.
     */
    ...
    /**
     * Handles the event when a new WebSocket connection is established. This
     * method is called when a client connects to the WebSocket server. It adds
     * the new connection to the set of active connections and logs the event.
     *
    ...
    /**
     * Handles incoming messages from WebSocket clients. This method is called
     * when a client sends a message to the WebSocket server. It logs the
     * received message, sends it to the Kafka topic using the ChatProducer, and
    ...
    /**
     * Handles the event when a WebSocket connection is closed. This method is
     * called when a client disconnects from the WebSocket server. It removes
     * the connection from the set of active connections and logs the event.
     *
     * @param session The WebSocket session associated with the closed
     * connection.
     */
    @OnClose
    ...
    /**
     * Broadcasts a message to all connected WebSocket clients. This method
     * iterates through all active WebSocket connections and sends the provided
     * message to each client. If an error occurs while sending the message to a
     * client, that client is removed from the set of active connections.
     *
    ...
    public static void broadcast(String message) {
        ...
        for (WebSocketServer client : connections) {
            try {
                synchronized (client) {
                    client.session.getBasicRemote().sendText(new Date() + " --> "
                            + message);
                }
            } catch (IOException e) {
